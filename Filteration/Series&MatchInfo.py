...
match_list = ['1327281', '1327233', '1335805',
              '1327280', '710291', '685735', '1000891']
data = {'MatchID': [], "SeriesID": [], 'MatchDate': [], 'EventName': [], 'EventStage': [], 'FieldUmpires_1': [], 'FieldUmpires_2': [], 'TVUmpire': [],
        'winner_team': [], 'ResultDescription': [], 'PlayerOfMatch': [], 'Season': [], 'TeamType': [], 'Team1': [], 'Team2': []}
...
def extract_matchinfo(match_id):
    ...
    return match_id, match_date, event_name, event_stage, field_umpires_1, field_umpires_2, tv_umpire, winner_team, result_description, player_of_match, season, team_type, team_1, team_2
...
def insert_match_info(match_list):
    series_db = pd.read_csv("series.csv")
    for matches in match_list:
        match_id, match_date, event_name, event_stage, field_umpires_1, field_umpires_2, tv_umpire, winner_team, result_description, player_of_match, season, team_type, team_1, team_2 = extract_matchinfo(
            matches)
        if int(matches) not in match_db['MatchID'].values:
            ...
            match_db.loc[len(match_db.index)] = [match_id, seriesid['SeriesID'].values[0], match_date, event_name, event_stage, field_umpires_1, field_umpires_2, tv_umpire,
                                                 winner_team, result_description, player_of_match, season, team_type, team_1, team_2]
    match_db.to_csv("Match.csv", index=False)


insert_match_info(match_list)
